...
//------------------------------------------------------
// single-precision axpby kernel incx == 1 && incy == 1 (FMA3)
// y = alpha * x + beta * y
// Uses FMA: y = fmadd(alpha, x, beta*y)
//------------------------------------------------------
void cblas_saxpby_k_noinc_fma(cblas_args_t* args)
{
    ...
    for (; i + 32 <= n; i += 32)
    {
        ...
        // y = alpha*x + beta*y using FMA: fmadd(alpha, x, beta*y)
        y0 = _mm256_fmadd_ps(alpha_vec, x0, _mm256_mul_ps(beta_vec, y0));
        y1 = _mm256_fmadd_ps(alpha_vec, x1, _mm256_mul_ps(beta_vec, y1));
        y2 = _mm256_fmadd_ps(alpha_vec, x2, _mm256_mul_ps(beta_vec, y2));
        y3 = _mm256_fmadd_ps(alpha_vec, x3, _mm256_mul_ps(beta_vec, y3));
    ...
    for (; i + 8 <= n; i += 8)
    {
        ...
        y_vec = _mm256_fmadd_ps(alpha_vec, x_vec, _mm256_mul_ps(beta_vec, y_vec));
...
//------------------------------------------------------
// double-precision axpby kernel incx == 1 && incy == 1 (FMA3)
// y = alpha * x + beta * y
// Uses FMA: y = fmadd(alpha, x, beta*y)
//------------------------------------------------------
void cblas_daxpby_k_noinc_fma(cblas_args_t* args)
{
    ...
    for (; i + 16 <= n; i += 16)
    {
        ...
        // y = alpha*x + beta*y using FMA: fmadd(alpha, x, beta*y)
        y0 = _mm256_fmadd_pd(alpha_vec, x0, _mm256_mul_pd(beta_vec, y0));
        y1 = _mm256_fmadd_pd(alpha_vec, x1, _mm256_mul_pd(beta_vec, y1));
        y2 = _mm256_fmadd_pd(alpha_vec, x2, _mm256_mul_pd(beta_vec, y2));
        y3 = _mm256_fmadd_pd(alpha_vec, x3, _mm256_mul_pd(beta_vec, y3));
    ...
    for (; i + 4 <= n; i += 4)
    {
        ...
        y_vec = _mm256_fmadd_pd(alpha_vec, x_vec, _mm256_mul_pd(beta_vec, y_vec));
