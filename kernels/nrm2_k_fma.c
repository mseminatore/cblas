...
//------------------------------------------------------
// single-precision nrm2 kernel incx == 1 (FMA)
//------------------------------------------------------
void cblas_snrm2_k_noinc_fma(cblas_args_t* args)
{
    ...
    for (; i + 8 <= n; i += 8)
    {
        ...
        sum_avx = _mm256_fmadd_ps(x_vec, x_vec, sum_avx);
    }
...
//------------------------------------------------------
// double-precision nrm2 kernel incx == 1 (FMA)
//------------------------------------------------------
void cblas_dnrm2_k_noinc_fma(cblas_args_t* args)
{
    ...
    for (; i + 16 <= n; i += 16)
    {
        ...
        // Use FMA: sum = x * x + sum
        sum0 = _mm256_fmadd_pd(x0, x0, sum0);
        sum1 = _mm256_fmadd_pd(x1, x1, sum1);
        sum2 = _mm256_fmadd_pd(x2, x2, sum2);
        sum3 = _mm256_fmadd_pd(x3, x3, sum3);
    }
    ...
    for (; i + 4 <= n; i += 4)
    {
        ...
        sum_avx = _mm256_fmadd_pd(x_vec, x_vec, sum_avx);
    }
