...
//------------------------------------------------------
// single-precision axpy kernel incx == 1 && incy == 1 (SSE)
// y = alpha * x + y
//------------------------------------------------------
void cblas_saxpy_k_noinc_sse(cblas_args_t* args)
{
    ...
    for (; i + 16 <= n; i += 16)
    {
        ...
        y0 = _mm_add_ps(_mm_mul_ps(alpha_vec, x0), y0);
        y1 = _mm_add_ps(_mm_mul_ps(alpha_vec, x1), y1);
        y2 = _mm_add_ps(_mm_mul_ps(alpha_vec, x2), y2);
        y3 = _mm_add_ps(_mm_mul_ps(alpha_vec, x3), y3);
    ...
    for (; i + 4 <= n; i += 4)
    {
        ...
        y_vec = _mm_add_ps(_mm_mul_ps(alpha_vec, x_vec), y_vec);
...
//------------------------------------------------------
// double-precision axpy kernel incx == 1 && incy == 1 (SSE)
// y = alpha * x + y
//------------------------------------------------------
void cblas_daxpy_k_noinc_sse(cblas_args_t* args)
{
    ...
    for (; i + 8 <= n; i += 8)
    {
        ...
        y0 = _mm_add_pd(_mm_mul_pd(alpha_vec, x0), y0);
        y1 = _mm_add_pd(_mm_mul_pd(alpha_vec, x1), y1);
        y2 = _mm_add_pd(_mm_mul_pd(alpha_vec, x2), y2);
        y3 = _mm_add_pd(_mm_mul_pd(alpha_vec, x3), y3);
    ...
    for (; i + 2 <= n; i += 2)
    {
        ...
        y_vec = _mm_add_pd(_mm_mul_pd(alpha_vec, x_vec), y_vec);
