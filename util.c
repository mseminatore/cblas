...
//------------------------------------------------------
// GEMM packing buffer pool - one buffer set per thread
//------------------------------------------------------
...
cblas_gemm_buffer_t* cblas_get_gemm_buffer(int thread_id)
{
    ...
    if (!gemm_buffer_pool[thread_id].allocated)
        return NULL;
    
    return &gemm_buffer_pool[thread_id];
}
